...
/**
 * Endpoint sekali pakai: nolkan kapasitas/durasi_jam acara arsip lama.
 *
 * Grup ACF "Detail Acara" didaftarkan dari dasbor, bukan dari kode, jadi
 * batas minimalnya tidak bisa dilonggarkan di sini. REST bawaan ACF juga
 * menolak 0 lewat skema otomatisnya (pesannya salah bunyi "inclusive" padahal
 * batasnya exclusive). update_field() lewat PHP tidak lewat skema itu sama
 * sekali, jadi dipakai di sini lewat endpoint sendiri yang sengaja sempit:
 * cuma dua field ini, cuma tipe acara, cuma pemegang akun yang berwenang
 * edit_posts. HAPUS blok ini sesudah kesepuluh draft arsip beres, ini bukan
 * alat permanen.
 */
add_action(
	'rest_api_init',
		...
		register_rest_route(
			...
			array(
				...
				'callback'            => function ( WP_REST_Request $req ) {
					...
					if ( 'acara' !== get_post_type( $id ) ) {
						return new WP_Error( 'bukan_acara', 'Bukan post acara.', array( 'status' => 404 ) );
					}

					update_field( 'kapasitas', 0, $id );
					update_field( 'durasi_jam', 0, $id );

					return array(
						'kapasitas'  => get_field( 'kapasitas', $id ),
						'durasi_jam' => get_field( 'durasi_jam', $id ),
					);
				},
			)
		);
